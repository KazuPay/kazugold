...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete KazuGold database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no KazuGold datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old KazuGold datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected KazuGold 0.7 datadir."
    ;;
  3)
    echo "Detected KazuGold pre-0.8 datadir."
    ;;
  4)
    echo "Detected KazuGold 0.8 datadir."
    ;;
esac
