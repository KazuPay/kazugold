...
###   This script attempts to download the signature file SHA256SUMS.asc from kazugold.org
...
WORKINGDIR="/tmp/kazugold"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://kazugold.org/bin/"
VERSIONPREFIX="kazugold-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'kazugold-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the KazuGold Core binary release signing key installed?"
   fi
