...
The RPM spec file provided here is for KazuGold-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://kazugold.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[KazuGold git repository](https://github.com/kazugold/kazugold) but are not part of
...
Currently only `Source10` is used, for the example `kazugold.conf` file.
...
in the kazugold git repository. At present only an SVG version of the KazuGold
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the kazugold
...
needed when 0.12.1 is released, a proper fix is already in the KazuGold git
master branch.
...
idea to build KazuGold Core against a static version of that library compiled
...
The `kazugold.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `kazugold-server` package. At this point, most RPM based
...
The `kazugold-server` package should have SELinux support. How to properly do
...
KazuGold-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/kazugold/). He has been
packaging KazuGold for Fedora at least since 2012.
...
4. I renamed the `kazugold` package that contains the Qt GUI to `kazugold-core` as
that appears to be how the general population refers to it, in contrast to
`kazugold-xt` or `kazugold-classic`. I wanted to make sure the general population
