...
// Copyright (c) 2017-2018 The KazuGold Core developers
...
#if defined(NDEBUG)
# error "Kazugold cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Kazugold Signed Message:\n";
...
void ThreadScriptCheck() {
    RenameThread("kazugold-scriptch");
    ...
    for (unsigned int i = 0; i < block.vtx[0].vout.size(); i++) {
        ...
        if(nAccValue > nPOWBlockReward && block.vtx[0].vout[i].nValue > 0) {
            ...
            if(metadata[nPaymentRequestsCount].isStr()) {
                ...
                if(block.vtx[0].vout[i].nValue != prequest.nAmount || prequest.fState != CFund::ACCEPTED || proposal.Address != CKazuGoldAddress(address).ToString())
...
//
// Commented - KazuGold uses now version control
//
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (pfrom->nVersion != 0)
    {
        ...
        {
            reason = "You are using an old version of KazuGold, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70017 && IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Segregated Witness has been enabled and you are using an old version of KazuGold, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70020 && IsCommunityFundEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Community Fund has been enabled and you are using an old version of KazuGold, please update.";
            fObsolete = true;
        }
