    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return KazuGoldUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, KazuGoldUnits::separatorNever);
            else
                return KazuGoldUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+KazuGoldUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
