...
#include "config/kazugold-config.h"
#endif

#include "main.h"
#include "kazugoldgui.h"
...
const std::string KazuGoldGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString KazuGoldGUI::DEFAULT_WALLET = "~Default";

KazuGoldGUI::KazuGoldGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void KazuGoldGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/kazugold"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a KazuGold address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kazugold: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your KazuGold addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified KazuGold addresses"));
    ...
    openAction->setStatusTip(tr("Open a kazugold: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible KazuGold command-line options").arg(tr(PACKAGE_NAME)));
...
void KazuGoldGUI::bootstrapBlockchain()
{
    bool ok = false;
    QString defaultUrl = "https://nav.nyc3.digitaloceanspaces.com/bootstrap/mainnet/bootstrap-kazugold_" +
            QString::fromStdString(Params().NetworkIDString()) + "net.tar";
    QString url = QInputDialog::getText(this, tr("Bootstrap blockchain"),
                                            tr("You can use an external trusted source to download the blockchain from.<BR>The following URL points to a bootstrap copy provided by the KazuGold Core Team.<BR>Where would you like to download it from?"), QLineEdit::Normal,
                                            defaultUrl, &ok);
    if (ok && !url.isEmpty())
    {
...
void KazuGoldGUI::createMenuBar()
{
#ifdef Q_OS_MAC
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(KazuGoldUnits::Unit u, KazuGoldUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(KazuGoldUnits::name(u)), this);
...
void KazuGoldGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void KazuGoldGUI::createToolBars()
{
    if(walletFrame)
    {
...
void KazuGoldGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool KazuGoldGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void KazuGoldGUI::startVotingCounter()
{
    if (GetStaking())
    {
...
bool KazuGoldGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void KazuGoldGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void KazuGoldGUI::setWalletActionsEnabled(bool enabled)
{
...
void KazuGoldGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void KazuGoldGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void KazuGoldGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void KazuGoldGUI::optionsClicked()
{
...
void KazuGoldGUI::showDebugWindow()
{
...
void KazuGoldGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void KazuGoldGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void KazuGoldGUI::openClicked()
{
...
void KazuGoldGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void KazuGoldGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuGoldGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuGoldGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuGoldGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuGoldGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KazuGoldGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void KazuGoldGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to KazuGold network", "", count));
    if(walletFrame){
...
void KazuGoldGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to KazuGold network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to KazuGold network..."));
...
void KazuGoldGUI::showVotingDialog()
{
...
void KazuGoldGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("KazuGold"); // default title
...
void KazuGoldGUI::changeEvent(QEvent *e)
{
...
void KazuGoldGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KazuGoldGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KazuGoldGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(KazuGoldUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void KazuGoldGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KazuGoldGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KazuGoldGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KazuGoldGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KazuGoldGUI::setEncryptionStatus(int status)
{
...
void KazuGoldGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KazuGoldGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KazuGoldGUI::detectShutdown()
{
...
void KazuGoldGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void KazuGoldGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(KazuGoldGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void KazuGoldGUI::subscribeToCoreSignals()
{
...
void KazuGoldGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<KazuGoldUnits::Unit> units = KazuGoldUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const KazuGoldUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(KazuGoldUnits::name(unit)));
    }
...
{
    setText(KazuGoldUnits::name(newUnits));
}
...
void KazuGoldGUI::toggleStaking()
{
...
void KazuGoldGUI::replyFinished(QNetworkReply *reply)
{
...
void KazuGoldGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CKazuGoldAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void KazuGoldGUI::getVotingInfo()
{
    QNetworkAccessManager *manager = new QNetworkAccessManager();
    QNetworkRequest request;
    QNetworkReply *reply = NULL;
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.kazugold.org/voting.") + QString::fromStdString(Params().NetworkIDString()) + QString("net.json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void KazuGoldGUI::updateStakingStatus()
{
    updateWeight();
