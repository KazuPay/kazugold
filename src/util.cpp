...
const char * const KGLDCOIN_CONF_FILENAME = "kazugold.conf";
const char * const KGLDCOIN_PID_FILENAME = "kazugold.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KazuGold
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KazuGold
    // Mac: ~/Library/Application Support/KazuGold
    // Unix: ~/.kazugold
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KazuGold";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KazuGold";
#else
    // Unix
    return pathRet / ".kazugold";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", KGLDCOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No kazugold.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override kazugold.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", KGLDCOIN_PID_FILENAME));
