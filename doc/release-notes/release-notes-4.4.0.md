...
- Fee to create a Proposal: 50 KGLD
...
You can read more about [NPIP0003 on the NPIP GitHub](https://github.com/KGLDCoin/npips/blob/master/npip-0003.mediawiki).

## Community Fund Contribution Increase

The Version Bit 16 will signal for the soft fork to increase the Community Fund contribution from 0.25 KGLD to 0.50 KGLD per block. 

You can read more about [NPIP0005 on the NPIP GitHub](https://github.com/KGLDCoin/npips/blob/master/npip-0005.mediawiki).
...
```
createproposal kazugoldaddress amount duration "desc" fee

Arguments:
1. "kazugoldaddress" (string, required) The kazugold address where coins would be sent if the proposal is approved.
2. "amount" The amount in KGLD to request. eg 100
...
2. "amount" (numeric or string, required) The amount in KGLD to withdraw. eg 10
...
Donate KGLD from your wallet to the commnuity fund

Arguments:
1. "amount" (string, required) The amount of KGLD to donate
```
...
### Other modifications in the KazuGold client:
